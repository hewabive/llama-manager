...
- `config/instances/<name>.json` — instance definitions (`instances/config-files.ts` file-per-instance store + in-memory cache; `instances/repository.ts` CRUD, signatures unchanged). Filename = `name` (charset `^[A-Za-z0-9._-]+$`), but a stable internal `id` lives in the body and remains the runtime key (`process_runs.instanceId`, supervisor map, proxy `target.instanceId`, `/api/instances/:id`); rename moves the file, id is preserved. Body = `Instance` minus runtime `status`/`pid` (derived on read from supervisor + `process_runs`). `binaryPath` is stored literally; `binaryPathRefId` (optional) re-resolves against `path_catalog` on read.
...
JSON files seed from git-tracked repo-root `config/*.json` (not `data/config/`) and fail loud on malformed JSON; runtime-computed defaults fill absent sections. On first run after upgrade, `config-relocation.ts` moves legacy `data/{settings.json,argument-defaults.json,presets/}` into `data/config/`, `proxy/legacy-migration.ts` exports the old SQLite proxy tables to files then drops them (`api_proxy_runtime_metadata` stays in the DB, rebuilt without its target FK), and `instances/legacy-migration.ts` exports the old `instances` table to `config/instances/*.json` (sanitizing names to the filename charset), rebuilds `process_runs` without its instances FK, then drops the table.
