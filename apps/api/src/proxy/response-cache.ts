...
const selectStatement = lazyStatement(
...
export function evictApiProxyResponseCache(): void {
  ...
  const rows = lruStatement().all() as Array<{
    key: string;
    sizeBytes: number;
  }>;
...
const statsStatement = lazyStatement(
