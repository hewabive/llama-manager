import type { LlamaArgumentCatalog, LlamaArgumentOption, LlamaArgumentValueType } from "@llama-manager/core";
...
const helpRuOverlay: Record<string, string> = {
  "--model": "Путь к GGUF-модели, которую должен загрузить экземпляр llama-server.",
  "--ctx-size": "Размер контекста в токенах. 0 означает взять значение из модели, если оно доступно.",
  "--n-gpu-layers": "Сколько слоев модели выгрузить в VRAM. Значение auto обычно является хорошим стартом, all пробует выгрузить все слои.",
  "--host": "Адрес, на котором llama-server будет слушать HTTP-запросы. Для доступа только с этого компьютера обычно достаточно 127.0.0.1.",
  "--port": "TCP-порт HTTP-сервера.",
  "--api-prefix": "Префикс URL без завершающего слеша, если сервер должен жить не в корне HTTP-пути.",
  "--parallel": "Количество серверных слотов для одновременной обработки запросов. -1 включает автоматический выбор.",
  "--batch-size": "Логический максимум batch size при обработке промпта. Влияет на производительность и потребление памяти.",
  "--ubatch-size": "Физический micro-batch size. Часто имеет смысл менять вместе с batch-size при нехватке памяти.",
  "--threads": "Количество CPU-потоков для генерации. -1 означает автоматический выбор.",
  "--threads-batch": "Количество CPU-потоков для batch/prompt processing. Если не задано, наследует --threads.",
  "--flash-attn": "Включение Flash Attention: on, off или auto. Может улучшить скорость и снизить память, если backend поддерживает.",
  "--cache-type-k": "Тип данных KV-cache для ключей. Меньшие типы экономят память, но могут влиять на качество/скорость.",
  "--cache-type-v": "Тип данных KV-cache для значений. Меньшие типы экономят память, но могут влиять на качество/скорость.",
  "--split-mode": "Стратегия распределения модели по нескольким GPU.",
  "--tensor-split": "Доли распределения модели по GPU, например 3,1.",
  "--main-gpu": "Основной GPU для режима split-mode=none или промежуточных результатов в split-mode=row.",
  "--mmproj": "Путь к multimodal projector для vision/multimodal моделей.",
  "--mmproj-auto": "Автоматически использовать mmproj, если он доступен, например при загрузке с Hugging Face.",
  ...
  "--models-max": "Максимум одновременно загруженных моделей в router-режиме. 0 означает без лимита.",
  ...
  "--cache-reuse": "Минимальный размер чанка для повторного использования prompt cache через KV shifting.",
  "--timeout": "Таймаут чтения/записи HTTP-сервера в секундах.",
  "--threads-http": "Количество потоков для обработки HTTP-запросов.",
  "--api-key": "API-ключи для аутентификации, можно передать несколько через запятую.",
  ...
  "--embedding": "Ограничить сервер embedding-сценарием. Используйте с dedicated embedding моделями.",
  "--rerank": "Включить endpoint reranking.",
  "--chat-template": "Задать Jinja-шаблон чата вручную вместо шаблона из метаданных модели.",
  ...
  "--reasoning-format": "Формат обработки thought-тегов и поля reasoning_content.",
  "--reasoning-budget": "Бюджет токенов для thinking: -1 без ограничения, 0 сразу завершить, N ограничить.",
  "--sleep-idle-seconds": "Через сколько секунд простоя сервер переводит модель в sleep; -1 отключает.",
  "--lora": "Путь к LoRA-адаптеру. Несколько адаптеров можно передать через запятую.",
  "--lora-scaled": "LoRA-адаптеры с пользовательским scale в формате FNAME:SCALE.",
...
function defaultBinaryPath() {
  ...
  const target = process.platform === "win32" && !settings.target.endsWith(".exe") ? `${settings.target}.exe` : settings.target;
  const buildCandidate = resolve(settings.buildDir, "bin", target);
  if (existsSync(buildCandidate)) {
    return buildCandidate;
  }

  const reffdevCandidate = resolve(config.rootDir, "..", "llama.cpp", "build-reffdev", "bin", target);
...
function runHelp(binaryPath: string) {
  ...
  const libraryPathName = process.platform === "darwin" ? "DYLD_LIBRARY_PATH" : "LD_LIBRARY_PATH";
  const libraryPath = [binaryDir, process.env[libraryPathName]].filter(Boolean).join(process.platform === "win32" ? ";" : ":");
  ...
  if (result.status !== 0) {
    throw new Error((result.stderr || result.stdout || `llama-server --help exited with code ${result.status}`).trim());
  }
...
function namesFromOptionText(optionText: string) {
  const matches = optionText.match(/(?:^|[\s,])-{1,2}[A-Za-z0-9][A-Za-z0-9_.-]*/g) ?? [];
...
function primaryName(names: string[]) {
  return names.find((name) => name.startsWith("--") && !name.startsWith("--no-")) ?? names.find((name) => name.startsWith("--")) ?? names[0]!;
}
...
function allowedValues(valueHint: string | null, help: string) {
  ...
  if (braced) {
    return braced[1]!.split(",").map((item) => item.trim()).filter(Boolean);
  }
  ...
  if (allowedLine) {
    return allowedLine[1]!.split(",").map((item) => item.trim()).filter(Boolean);
  }
...
function inferValueType(input: {
  ...
  const hasNegation = input.names.some((name) => name.startsWith("--no-") || name.startsWith("-no"));
  ...
  if (input.allowedValues.length > 0) {
    return input.allowedValues.every((value) => ["on", "off", "auto", "0", "1", "true", "false"].includes(value))
      ? "boolean"
      : "enum";
  }
  ...
  if (/^(n|port|index|seconds|similarity|seed|start|end|mib0,mib1,mib2,\.\.\.)$/i.test(input.valueHint ?? "")) return "number";
  if (hint.includes(",") || help.includes("comma-separated")) return "list";
  if (hint === "<0|1>" || hint === "[on|off]" || hint === "[on|off|auto]") return "boolean";
  return "string";
}
...
function helpWithoutEnv(help: string) {
  return help.replace(/\(env:\s*[^)]+\)/g, "").replace(/\s+/g, " ").trim();
}
...
function toOption(parsed: ParsedHelpOption): LlamaArgumentOption | null {
  ...
  const category = categoryNamesRu[parsed.category] ?? parsed.category;
  ...
  return {
    ...
    category,
    valueHint,
    valueType: inferValueType({ names, valueHint, allowedValues: values, help }),
