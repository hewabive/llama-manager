...
После обычного LCP reuse сервер ведет два курсора с позиции `n_past`: `head_c` по старому prompt слота и `head_p` по новому. Ран одинаковых токенов длиной не меньше `n_cache_reuse` сдвигается на новую позицию (`common_context_seq_rm()` + `common_context_seq_add()`, `kv_shift = head_p - head_c` всегда ≤ 0), «дыра» между чанками удаляется из KV; более короткий ран пропускается (`head_c++`). Выигрывает первый ран длиной не меньше порога, а не самый длинный. При speculative decoding те же KV-сдвиги зеркалируются в draft-контекст (`ctx_dft`).

`head_p` продвигается только при успешном совпадении, поэтому reuse срабатывает только когда новый prompt получается из старого удалением кусков (скользящее окно кода в FIM, выпавшая секция RAG, усеченная история). Вставка нового текста в середину обрывает цепочку: с точки вставки и до конца prompt пересчитывается честно. Это структурное ограничение: инвариант `head_c >= head_p` допускает только сдвиги влево, а хвост после вставки потребовал бы сдвига вправо — он не переиспользуется, каким бы длинным ни был; выживает только общий префикс.
...
- `0`: отключить.
- Положительное число: минимальная длина chunk в токенах — порог, отделяющий настоящий повторившийся блок от случайного совпадения рана токенов (переносы строк, boilerplate-форматирование); при маленьком N первый случайный ран проходит порог раньше настоящего блока и переиспользует KV из чужого контекста. На префикс N не влияет: LCP переиспользуется всегда, независимо от этого флага.
...
Выбор N: нижняя граница отсекает случайные совпадения и микро-выгоду — пропуск prefill 16-32 токенов экономит миллисекунды, а каждый шов добавляет ошибку, поэтому значения ≤ 32 практического смысла не имеют; верхняя граница — N не должен превышать типичный повторяющийся блок нагрузки (функция, секция документа), иначе ни один chunk не пройдет порог. Ориентир upstream — `256`: его ставят все пресеты `--fim-qwen-*` (неожиданный side effect, если вы запускаете сервер через такой пресет) и llama.vim. `64` — агрессивное значение для кода/RAG с мелкими блоками; начинайте с 256 и снижайте, наблюдая `timings.cache_n` и trace-логи. Если блоки крупные и жестко структурированные, ставьте N близко к размеру минимального блока — случайные раны порог не пройдут, а настоящие блоки пройдут все.

Паттерн для запросов с разными сочетаниями блоков: прогрейте кэш полным набором блоков (запрос с `n_predict` 0-1) — любое подмножество тогда становится deletion-shaped диффом; собрать контекст из нескольких записей кэша нельзя, чанки берутся из одной предыдущей последовательности. Условия: блоки в запросах сохраняют порядок суперсета (перестановку reuse не покрывает); первый блок константный, так как выбор слота и восстановление из RAM prompt cache считают похожесть только по общему префиксу (`get_common_prefix`); суперсет не сильно больше типичного запроса — `prompt_load()` игнорирует записи с `f_keep < 0.25` (LCP / размер записи). Учитывайте, что восстановление потребляет запись кэша: после первого запроса-подмножества суперсет исчезает из RAM prompt cache, слот хранит «подмножество + ответ», и для следующей комбинации прогрев нужно повторять.
