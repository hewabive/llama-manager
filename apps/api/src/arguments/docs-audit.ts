...
reviewedLlamaCppCommit: ${yamlString(input.llamaCppCommit)}
...
- Поведение отличается после \`git pull\`: запустите \`pnpm --filter @llama-manager/api args:docs:source-sync -- --diff\` и проверьте, изменился ли generated help-блок.
...
const sourceStatus = getLlamaSourceStatus();
const sourceCommit =
  sourceStatus.exists && sourceStatus.isGitRepo
...
const report = {
  sourceRepoPath: sourceStatus.settings.repoPath,
  sourceCommit,
