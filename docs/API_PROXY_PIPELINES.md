...
| type              | config                                                                                                      | ports                         |
| ----------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------- |
| `replace-text`    | `rules: [{enabled, mode, find, replace}]` — `mode: "text"` (default) replaces inside string values: JSON-style escapes `\n` `\t` `\"` `\\` `\uXXXX` are decoded before matching so text copied from a capture file matches as-is; the routing `model` field is never rewritten. `mode: "json"` edits structure: `find`/`replace` are JSON fragments — either a list of values (`{...}, {...}` matches a contiguous window in any array, e.g. messages; empty `replace` removes it) or `"key": value` entries (matches any object containing them all; matched keys are removed, `replace` entries merged). Matching is structural (deep equality), formatting-insensitive; invalid fragments are rejected at save time and skipped at runtime. | `next`                        |
