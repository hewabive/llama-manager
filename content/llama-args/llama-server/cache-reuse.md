...
title: "--cache-reuse"
summary: "Минимальный размер чанка для повторного использования prompt cache через KV shifting."
docStatus: draft
...
```text
min chunk size to attempt reusing from the cache via KV shifting, requires prompt caching to be enabled (default: 0) [(card)](https://ggml.ai/f0.png)
```
...
- Переменные окружения: `LLAMA_ARG_CACHE_REUSE`
...
```bash
llama-server --model /models/example.gguf --cache-reuse 1
```
...
- https://github.com/ggml-org/llama.cpp/search?q=--cache-reuse&type=code
- https://github.com/ggml-org/llama.cpp/issues?q=--cache-reuse
- https://github.com/ggml-org/llama.cpp/discussions?discussions_q=--cache-reuse
