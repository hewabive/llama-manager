...
title: "--ctx-size"
summary: "Размер контекста в токенах. 0 означает взять значение из модели, если оно доступно."
docStatus: draft
...
- Переменные окружения: `LLAMA_ARG_CTX_SIZE`
- Значение по умолчанию из `--help`: `0, 0 = loaded from model`
...
```bash
llama-server --model /models/example.gguf --ctx-size 1
```
...
- https://github.com/ggml-org/llama.cpp/search?q=--ctx-size&type=code
- https://github.com/ggml-org/llama.cpp/issues?q=--ctx-size
- https://github.com/ggml-org/llama.cpp/discussions?discussions_q=--ctx-size
